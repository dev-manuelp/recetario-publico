...
/**
 * PÁGINA DE CREACIÓN DE RECETAS - ESCANEO Y EDICIÓN
 */
...
import { Camera, Save, Loader2, Trash2, ImagePlus, RotateCcw, Palette, Plus, ChefHat, ArrowLeft } from "lucide-react";
...
const FONTS = [
  ...
  // { id: 'italianno', name: 'Fina y Alta', class: 'font-italianno' },
...
export default function NewRecipePage() {
  ...
  const addStepAfter = (index: number) => {
    ...
    // splice(posición, 0 borrados, elemento a añadir)
  ...
  return (
    <div className="p-4 w-full mx-auto max-w-md md:max-w-3xl pb-40 min-h-screen bg-[#FBF7F4] relative">

      {/* BLOQUE INICIAL: CAPTURA DE PÁGINAS FÍSICAS */}
      {!recipe && !loading && (
        <div className="text-center py-10 relative">

          <div className="absolute top-0 left-0">
            <Link href="/">
              <button className="flex items-center justify-center bg-white/80 backdrop-blur-sm text-stone-700 h-10 w-10 rounded-full shadow-sm border border-stone-100 active:scale-95 transition-transform">
          ...
          {recipePages.length === 0 && (
            <p className="text-xl font-sans mb-4 text-stone-600 font-bold mt-12">Haz fotos a las páginas de la receta</p>
          )}

          {recipePages.length > 0 && (
            <div className="flex gap-4 overflow-x-auto p-4 mb-6 justify-center no-scrollbar mt-12">
              ...
              <div className="relative w-24 h-32 flex-shrink-0 border-2 border-dashed border-stone-300 rounded-lg flex flex-col items-center justify-center text-stone-400 hover:bg-stone-50 cursor-pointer">
                <Plus className="w-8 h-8 mb-1" />
                <span className="text-xs font-bold uppercase tracking-tighter">Añadir pág.</span>
                <input type="file" accept="image/*" capture="environment" className="absolute inset-0 opacity-0" onChange={handleAddPage} />
              </div>
            </div>
          )}
          ...
          {recipePages.length === 0 ? (
            <div className="relative inline-block group mt-8">
              <div className="absolute -inset-1 bg-gradient-to-r from-orange-600 to-amber-600 rounded-full blur opacity-30 group-hover:opacity-50 transition duration-200"></div>
              <Button size="lg" className="relative h-24 w-24 rounded-full shadow-2xl bg-gradient-to-r from-orange-500 to-amber-600 border-4 border-[#FBF7F4]">
                <Camera className="w-10 h-10 text-white" />
              </Button>
              <input type="file" accept="image/*" capture="environment" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20" onChange={handleAddPage} />
            </div>
          ) : (
            <Button onClick={startScanning} className="h-16 px-8 rounded-full bg-gradient-to-r from-orange-600 to-amber-600 text-white font-bold text-lg shadow-xl hover:scale-105 transition-transform">
              <span className="mr-2">✨</span> Analizar {recipePages.length} pág{recipePages.length > 1 ? 's' : ''}
            </Button>
          )}
        </div>
      )}
      ...
      {recipe && !loading && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <Card className="border-2 border-stone-200 bg-white shadow-xl mb-24 rounded-[2rem] overflow-hidden">
            <CardContent className={cn("p-6 md:p-10 text-xl leading-relaxed", selectedFont.class)}>
              ...
              <div className="mb-4">
                ...
                <div className="space-y-4">
                  {recipe.pasos.map((paso, i) => (
                    <div key={i} className="flex gap-3 items-start">
                      ...
                      />
                      {/* ESTO ES LO NUEVO: DOS BOTONES (Borrar e Insertar) */}
                      <div className="flex flex-col gap-1 mt-2">
                        ...
                        {/* Botón Insertar Debajo */}
                        <button
                          onClick={() => addStepAfter(i)}
                          className="p-1 text-stone-300 hover:text-green-600 transition-colors"
                ...
                <button
                  onClick={addStep}
                  className="mt-4 flex items-center gap-2 text-orange-600 font-bold text-sm px-4 py-2 hover:bg-orange-50 rounded-lg transition-colors font-sans w-full justify-center"
                >
          ...
          <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-md border-t border-stone-200 z-50 flex gap-4">
            <button onClick={() => setRecipe(null)} className="flex-1 h-14 rounded-2xl border border-stone-200 font-bold bg-white text-stone-600 active:scale-95 transition-all">Repetir</button>
